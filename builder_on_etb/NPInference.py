...
import os
import numpy
from NPEnum import DataAttribute
from typing import Tuple, List, Dict, NewType
from NPError import AlreadyInitializedError, UninitializedError
...
class SilentLogger(tensorrt.Logger):
...
def model_initialize(cls, **kwargs)->None:
    ...
    TRT_LOGGER = SilentLogger(tensorrt.Logger.ERROR)
    runtime = tensorrt.Runtime(TRT_LOGGER)
    print(kwargs.get("engine_path"))
    with open(kwargs.get("engine_path"), "rb") as f:
...
def has_interpreter_context(cls):
    return True if __interpreter_dict[cls].get(INTERPRETER, None) is not None else False
